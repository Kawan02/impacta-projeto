...
class EditContatosPage extends StatelessWidget {
  ...
  ImageProvider<Object> imagem(String? imagem, String? imagemSelecionada) {
    ...
    if (imagemSelecionada.isNotEmpty) {
      String? nome = controllerNome.text.isEmpty ? model.nome : controllerNome.text;
      removeStorageImage(model.nome!);
  ...
  Future<void> atualizarContato(int id, EditContatoController controller, ContatosModel contatosModel) async {
    ContatosModel model = ContatosModel(
      // id: id,
      nome: controllerNome.text,
      favorito: controller.favorito.value,
      image: controller.image.value.isEmpty ? contatosModel.image! : controller.image.value,
      sobrenome: controllerSobrenome.text,
      telephone: controllerTelefone.text,
      amigo: controller.selectedItem.value.isEmpty ? contatosModel.amigo : controller.selectedItem.value,
      dtaNascimento: controllerDtaNascimento.text,
      nota: controllerNotas.text,
      updateAt: date.DateFormat('yyyy-MM-dd:kk:mm').format(DateTime.now()),
    );

    if (_formKey.currentState!.validate()) {
      await controller.putContato(model);
      // String? nome = controllerNome.text.isEmpty ? model.nome : controllerNome.text;
      // removeStorageImage(model.nome!);
      if (controllerNome.text.isNotEmpty) {
        return saveStorageImage(base64Decode(controller.image.value), controllerNome.text);
      } else {
        return saveStorageImage(base64Decode(controller.image.value), model.nome!);
      }
    }
  }
                          ...
                          GetBuilder<EditContatoController>(
                            ...
                            builder: (controller) {
                              return FloatingActionButton.small(
                                ...
                                onPressed: () {
                                  controller.image.value = "imageDefault";

                                  String? nome = controllerNome.text.isEmpty ? model.nome : controllerNome.text;
                                  removeStorageImage(nome!);
                                },
                              );
                            },
                          ),
                          ...
                          children: [
                            SizedBox(
                              ...
                              child: ElevatedButton.icon(
                                ...
                                onPressed: () async {
                                  await atualizarContato(model.id!, controller, model);
                                },
                                icon: Visibility(
